...
class Exemplar(Density_Model):
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.Variable(tf.zeros(int(z_size)))
    ...
    def make_prior(self, z_size):
        ...
        """
        prior_mean = tf.zeros(int(z_size))
        prior_logstd = tf.zeros(int(z_size))
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run([self.likelihood], 
                                   feed_dict={self.state1: state1, self.state2: state2, self.discrim_target: discrim_targ})[0]
        return likelihood
