...
class ACAgent(BaseAgent):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        V = self.sess.run([self.critic.critic_prediction], feed_dict={self.critic.sy_ob_no: ob_no})[0]
        Q = re_n + self.gamma * (1 - terminal_n) * self.sess.run([self.critic.critic_prediction], feed_dict={self.critic.sy_ob_no: next_ob_no})[0]
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for _ in range(self.agent_params['num_actor_updates_per_agent_update']):
            ac_loss = self.actor.update(ob_no, ac_na, adv_n)
